...
function EpisodeWorkspace({ session, episode, number, ready, onBack }: { session: ProjectSession; episode: Episode; number: number; ready: boolean; onBack: () => void }) {
  const [value, setValue] = useState(() => readWorkflow(session.projectId, episode.id, { aspect: session.project.aspect }));
  ...
  function update(change: EpisodeWorkflow | ((v: EpisodeWorkflow) => EpisodeWorkflow)) {
    ...
    setSaveMessage(result.ok ? '已自动保存' : '未保存');
  ...
  return <div className="episode-page web-episode">
    ...
    <div className="episode-layout">
      ...
      <div className="episode-content">
        <div className="episode-demo-banner" role="note"><span>演示空间 · 所有生成操作均为示例，不调用 AI 服务。</span><span>{current + 1} / {episodeStages.length}</span></div>
        ...
        <section hidden={step !== 'script'} id="episode-stage-script" data-testid="episode-stage-script" className="episode-stage" aria-label="剧本确认与素材拆解"><ScriptStage value={value} readOnly={readOnly} onChange={update} projectAspect={session.project.aspect}/></section>
