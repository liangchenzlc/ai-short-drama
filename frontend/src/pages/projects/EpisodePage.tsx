...
function EpisodeWorkspace({ session, episode, number, ready, onBack }: { session: ProjectSession; episode: RemoteEpisode; number: number; ready: boolean; onBack: () => void }) {
  ...
  const getStoryboardBarrier = useRef(() => storyboardBarrier.current).current;
  ...
  const writingLabel = { loading: '正在载入服务端内容…', saved: '小说与剧本已保存至服务端', unsaved: '有未保存的修改', saving: '正在保存…', error: '保存已暂停', conflict: '版本冲突 · 保存已暂停' }[writing.status];
  ...
  return <div className="episode-page web-episode">
    <header className="episode-top"><Button type="link" className="detail-back" onClick={onBack}>← 返回项目详情</Button><div><span>{session.project.name} / 第 {number} 集</span><h1>{episode.title}</h1></div><span className={writing.message ? 'episode-save-error' : 'episode-top-status'} role={writing.message ? 'alert' : 'status'}>{writingLabel}</span></header>
    {readOnly && <p className="episode-readonly-notice">当前为只读模式，可查看本集内容。</p>}
    <div className="episode-layout">
      <aside className="episode-sidebar"><div className="episode-sidebar-inner"><p>本集制作</p><StageNav active={step} onSelect={goToStep}/><div className="episode-context-summary"><strong>{value.aspect} 画幅</strong><span>素材与分镜由服务端保存</span><p>正文停顿后自动保存；生成与采用需要明确操作。</p></div></div></aside>
      <div className="episode-content">
        <div className="episode-demo-banner" role="note"><span>{step === 'video' ? `视频阶段保留为后续范围。${saveError || saveMessage}` : '制作数据保存至服务端 · 生成结果需预览并明确采用'}</span><span>{current + 1} / {episodeStages.length}</span></div>
