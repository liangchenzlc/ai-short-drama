...
export function ImagePicker({ busy, onClose, onSelect }: {
  ...
  return <Dialog title="从图片资产库选择" className="media-picker-dialog" canClose={!busy && !choosing} onClose={onClose}>
    <div className="media-picker-body">
      ...
      {loading ? <Skeleton paragraph={{ rows: 4 }}/> : items.length ? <div className="media-picker-grid">{items.map(item => <button className="media-picker-item" key={item.asset_id} disabled={busy || !!choosing || !item.url} onClick={() => void choose(item)}>
        {item.url ? <img src={item.url} alt="" loading="lazy"/> : <span className="media-picker-unavailable">预览暂不可用</span>}<strong>{choosing === item.asset_id ? '正在添加…' : item.name}</strong>
      </button>)}</div> : !error && <div className="studio-empty"><h3>{query ? '没有找到图片' : '图片资产库还是空的'}</h3><p>{query ? '试试其他名称，或清空搜索。' : '可以关闭此窗口，直接上传本地图片。'}</p></div>}
