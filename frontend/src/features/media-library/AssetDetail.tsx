...
export function AssetDetail({ id, onClose, onChanged }: { id: string; onClose: () => void; onChanged: () => void }) {
  ...
  async function apply(values: ApplyValues) {
    if (!asset || !confirmed || applyConflict || mutation.current) return;
    ...
    try {
      ...
      await mediaLibrary.apply(id, { target: { type: values.type, id: values.id.trim() }, expected_media_id: values.expected_media_id?.trim() || null,
  ...
  return <Drawer open title="资产详情" width={760} onClose={() => !busy && onClose()} closable={!busy} maskClosable={!busy} keyboard={!busy} rootClassName="generation-drawer">
    ...
    {loading && !asset ? <div className="generation-loading"><Spin tip="加载资产…"><div /></Spin></div> : asset && <>
      ...
      <section className="generation-section"><h3>资产信息</h3>
        ...
        <dl className="generation-facts"><div><dt>媒体 ID</dt><dd className="generation-id">{asset.media_id}</dd></div><div><dt>尺寸</dt><dd>{asset.width && asset.height ? `${asset.width} × ${asset.height}` : '未提供'}</dd></div>{asset.media_type === 'video' && <div><dt>时长</dt><dd>{asset.duration_ms != null ? `${asset.duration_ms / 1000} 秒` : '未提供'}</dd></div>}<div><dt>入库时间</dt><dd>{dateLabel(asset.created_at)}</dd></div><div><dt>来源任务</dt><dd><Link to={`/tasks/${asset.media_type}?task=${asset.generation_id}`}>{asset.generation_id}</Link></dd></div>{asset.source && <div><dt>来源分镜</dt><dd className="generation-id">{asset.source.shot_id}</dd></div>}</dl>
      </section>
      <section className="generation-section"><h3>采用到业务目标</h3><p className="generation-hint">资产已永久保存，只有确认采用后才会替换目标内容。当前项目页使用浏览器本地演示数据；这里必须填写真实服务端目标 ID。</p>
        {!applyOpen ? <Button type="primary" disabled={!!busy} onClick={() => { setApplyOpen(true); setConfirmed(false); setApplyError(''); form.setFieldsValue({ type: asset.media_type === 'video' ? 'shot_video' : 'shot_image' }); }}>选择目标并确认采用</Button> :
          <Form form={form} layout="vertical" onFinish={apply} disabled={!!busy} onValuesChange={() => { setConfirmed(false); setApplyConflict(false); }}>
            ...
            <Form.Item name="expected_media_id" label="目标当前媒体 ID（无媒体则留空）" extra="用于避免覆盖他人的修改。若已有媒体，请填写当前媒体 ID；不要填写资产 ID。" rules={[idRule]}><Input placeholder="已确认目标没有媒体时留空" /></Form.Item>
            ...
            <Checkbox checked={confirmed} disabled={!!busy || applyConflict} onChange={(event) => setConfirmed(event.target.checked)}>我已核对真实目标和当前媒体，确认采用此资产。</Checkbox>
