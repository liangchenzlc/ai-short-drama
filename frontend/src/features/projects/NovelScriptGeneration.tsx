...
export function NovelScriptGeneration({
  projectId, episodeId, modelId, novel, disabled, session, modelSelector, onEditScript,
}: {
  projectId: string;
  episodeId: string;
  ...
  async function generate() {
    ...
    try {
      ...
      setMessageType('success'); setMessage('已开始生成。你可以继续编辑，结果会自动出现在下方。');
  ...
  async function select(item: ScriptCandidate) {
    ...
    try {
      ...
      else { setPreview(null); setScriptRevision((revision) => revision + 1); onEditScript(); }
    } catch (cause) { setMessageType('error'); setMessage(errorMessage(cause)); }
    finally { setBusy(false); }
  }
  ...
  return <>
    ...
    <section className="writing-candidates" aria-label="候选剧本">
      ...
      {loading ? <Skeleton title paragraph={{ rows: 2 }}/> : items.length ? <div className="script-candidate-list">{items.map(item => <article className={`script-candidate${item.is_editing ? ' is-current' : ''}`} key={item.id}>
        ...
        <div className="candidate-actions"><Button disabled={busy} onClick={() => void show(item)}>预览全文</Button>{item.is_editing && <Button onClick={onEditScript}>继续编辑</Button>}</div>
