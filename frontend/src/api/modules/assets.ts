...
export interface AssetCandidateGeneration {
  ...
  is_stale: boolean;
  stale_reason: 'content_changed' | 'source_mismatch' | 'snapshot_missing' | null;
}
export interface AssetCandidate { id: string; media_id: string; url: string; width?: number | null; height?: number | null; created_at: string; generation: AssetCandidateGeneration | null }
...
export const assetLibraries = {
  ...
  async candidates(id: string, signal?: AbortSignal, offset = 0, limit = 20) { return (await http.get<Page<AssetCandidate>>(`/assets/${encodeURIComponent(id)}/image-candidates`, { params: { offset, limit }, signal })).data; },
  ...
  async confirm(id: string, body: { row_version: string; media_id: string; expected_media_id: string | null; confirm_shared: boolean; acknowledge_stale_source?: boolean }) { return (await http.post<AssetRead>(`/assets/${encodeURIComponent(id)}/confirm`, body)).data; },
};
