...
export interface ShotRead extends WorkflowShot { position: number; script: string; asset_ids: string[]; image: ShotImage | null; deleted_at: string | null }
...
export function storyboardApi(projectId: string, episodeId: string) {
  ...
  return {
    ...
    async update(id: string, body: { row_version: string; script?: string; asset_ids?: string[]; image_settings?: ShotRead['image_settings'] }) { return (await http.patch<ShotMutation>(`${root}/shots/${encodeURIComponent(id)}`, body)).data; },
