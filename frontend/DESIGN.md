...
- 项目详情组织基本信息、分集和项目资源；项目设置使用显式保存。
- 分集一次展示一个制作步骤，侧边/窄屏横向导航说明当前任务；下一步不代表上一阶段已经确认。
- 小说编辑与改编设置在桌面双列，候选结果保持靠近正文；窄屏顺序堆叠。
- 素材直接展示角色/场景/道具分类，详情区分文字字段、参考图与候选采用。
- 分镜展示编号、时长、正文和素材关联；原文依据与历史任务可折叠。当前分镜页面只提供图片生成，不展示已移除的演示视频栏。
- 任务与资产列表把常用搜索/状态优先展示，进阶筛选收起；无数据与无搜索结果分别说明。
...
素材提取设置、任务状态和候选核对在同一弹窗切换；展示叙事作用、原文依据与重复提示，允许分项选择。长字段可展开，关闭前处理未保存修改，不强制弹出已完成任务。
