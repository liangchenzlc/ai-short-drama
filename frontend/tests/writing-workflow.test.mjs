...
test('stage view uses current server writing and confirmation instead of stale browser approvals',()=>{
...
test('local workflow persistence retains shot data and model preferences without copying server writing',()=>{
  const old=local();const view=projectWritingWorkflow(old,{loaded:true,novel:'server novel',script:'server script',confirmed:true});
