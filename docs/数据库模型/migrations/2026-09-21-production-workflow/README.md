...
本目录只交付迁移，不代表任何业务库已经执行。先对数据库元数据和 `episodes`、`shot_scripts`、`shot_assets`、`shot_images`、`shot_videos`、`script_shot_records`、`media_recycle_bin`、`assets`、`media_files` 做可恢复备份，并记录备份位置与时间。

要求 MySQL 8.0.16 或更高版本。迁移窗口必须暂停 API 写入、Worker 的业务保存与媒体采用；读取可以继续。使用同一个 `mysql` 客户端连接依次 `SOURCE` 以下 SQL，使 `000` 创建的临时基线表保留到 `005`：
...
5. 在仓库 `backend` 目录运行 `uv run python ../docs/数据库模型/migrations/2026-09-21-production-workflow/004_backfill_asset_candidates.py --database-url "$DATABASE_URL"` 预览；数量正确后增加 `--apply`。脚本分批提交、已有组合跳过，坏媒体引用会停止。
