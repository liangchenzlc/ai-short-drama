...
本目录用于在现有 `ai_short_drama` 数据库新增三张表及一个配置字段。使用 MySQL 8.0.21+；已有 `ai_model_configs`、`media_files` 必须与当前项目表结构兼容。无需重新执行包含原有 17 张表的 `schema.mysql8.sql`。
...
本批脚本从[表设计文档](../../通用异步任务与AI生成记录表设计.md)的增量 DDL 拆分而来。归档独立预算、投递暂停与安全恢复已在应用层实现，不需要增加表字段，运行规则见[模型生成运行说明](../../../模型生成运行说明.md)。
